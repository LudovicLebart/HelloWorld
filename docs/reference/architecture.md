...
| `logSpiral.ts` | `sampleLogSpiral()` : échantillonne une spirale logarithmique en repère local (origine au point de départ, rayon décroissant) — la forme d'une volute de rinceau. Retourne aussi la tangente de départ, pour que l'appelant puisse raccorder la spirale à une tangente donnée. |
| `branching.ts` | Génération automatique de branches secondaires : `planAutoBranches()` choisit les points d'accroche le long d'une tige (densité proportionnelle à la longueur, espacement perturbé par le nombre d'or) — le côté (gauche/droite) suit le sens de courbure locale de la tige (`localCurvature()`), jamais une simple alternance : une branche se détache toujours du côté convexe d'un virage, jamais du côté concave (le creux), même principe qu'en art du bonsaï ; sur un tronçon localement droit, retombe sur une alternance simple. `buildAutoBranchPoints()` construit les points bruts d'une volute (raccord tangent à `AUTO_BRANCH.launchAngle` de la tige parente, taille décroissant géométriquement le long de la tige) — voir [Ce qui rend une arabesque gracieuse](../explanation/principes-esthetiques.md). Les points produits repassent par `autoHandles()`/`nodesFromClicks()` comme n'importe quel tracé : les branches générées restent des lianes normales, éditables comme les autres. |
