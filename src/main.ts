...
/** Génère des volutes secondaires automatiquement sur la liane sélectionnée, en branches du même
    ...
    docs/explanation/principes-esthetiques.md pour les règles appliquées. Remplace le lot généré par
    un clic précédent sur cette même liane plutôt que de l'empiler (voir collectAutoDescendants). Un
    seul pas d'annulation pour tout le lot (retrait de l'ancien compris), pas un par volute. */
function generateAutoBranches(): void {
  ...
  pushUndo();
  for (const id of collectAutoDescendants(selectedId)) {
    vines.delete(id);
    renderer.removeVine(id);
  }
  const { shape, recursionDepth } = autoBranchParams();
  spawnAutoBranches(plans, selectedId, vine.params.stemWidth, 0, recursionDepth, shape);
  saveSnapshot();
}
