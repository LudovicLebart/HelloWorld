...
function liveParams(): VineParams {
  return {
    ...
    // Jamais d'amincissement à la base — qu'il s'agisse de la racine d'une branche (qui doit
    // s'enfoncer pleine largeur dans sa tige parente) ou de la base de la tige principale
    // (qui émerge "du sol" pleine largeur, comme une vraie tige de plante) : une liane s'effile
    // uniquement vers sa pointe, jamais à son point de départ — forme de triangle/cône, pas de
    // fuseau à deux pointes.
    taperStart: false,
    taperEnd: true,
  };
}
...
function selectVine(id: string): void {
  ...
  nodeEditor.show(vine.nodes, {
    ...
    onChange: () => regenerateAndRender(id, liveParams()),
...
function toggleNodeCorner(id: string, index: number): void {
  ...
  regenerateAndRender(id, liveParams());
  selectVine(id);
  saveSnapshot();
}
...
function addVine(nodes: EditableNode[], parentId?: string, autoGenerated = false): string {
  ...
  const params: VineParams = {
    ...base,
    stemWidth: autoGenerated ? base.stemWidth * AUTO_BRANCH.stemWidthFactor : base.stemWidth,
    autoGenerated,
  };
...
function refreshSelectedVine(): void {
  ...
  const vine = vines.get(selectedId);
  if (!vine) return;
  regenerateAndRender(selectedId, liveParams());
}
